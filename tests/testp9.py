...
class TestP9(TestCase):
    def __init__(self):
        ...
        self.excepted_center_edge_nodes_positions = [(3, 1.5), (3, -1.5), (-3, 1.5), (-3, -1.5), (0, 3), (0, -3)]
    ...
    def proper_graph_apply(self):
        ...
        if self.production.check(hyper_graph, hyper_node):
            ...
            new_center_edge_nodes_positions = self.excepted_center_edge_nodes_positions + [(0, 0)]
    ...
    def check_in_bigger_graph(self):
        ...
        expected_new_nodes = [(f'z{i}', {'pos': pos, 'h': True}) for i, pos in enumerate(self.excepted_center_edge_nodes_positions)]
        expected_new_edges = [
        ...
        expected_graph._remove_edges([{'a', 'b', 'c', 'd', 'e', 'f'}, {'a', 'b'},
                                    {'b', 'c'}, {'c', 'd'}, {'d', 'e'}, {'e', 'f'}, {'f', 'a'}])
        
        expected_edges_with_hnodes = []
        for edge, h_node in zip([{'a', 'b'}, {'b', 'c'}, {'c', 'd'}, {'d', 'e'}, {'e', 'f'}, {'f', 'a'}], ['z2', 'z4', 'z0', 'z1', 'z5', 'z3']):
            first_node = edge.pop()
            second_node = edge.pop()
            expected_edges_with_hnodes.append(({first_node, h_node}, {'label': f'g{len(expected_edges_with_hnodes)}', 'R': True}),)
            expected_edges_with_hnodes.append(({second_node, h_node}, {'label': f'g{len(expected_edges_with_hnodes)}', 'R': True}),)


        expected_graph.extend(nodes=expected_new_nodes + [('z', {'pos': (0, 0), 'h': False})],
                                    edges=[({f'z{i}', 'z'}, {'label': 'E', 'B': False}) for i in range(6)] + expected_new_edges + expected_edges_with_hnodes)
        ...
        bigger_graph.visualize()
        self.assertTrue(isomorphism.GraphMatcher(bigger_graph.nx_graph, expected_graph.nx_graph).is_isomorphic())
    ...
    def test_with_boundary(self):
        ...
        if self.production.check(hyper_graph, hyper_node):
            ...
            modified_graph_nodes_positions = [params['pos'] for _, params in hyper_graph.nodes]
            self.assertSetEqual(
                set(self.excepted_center_edge_nodes_positions  + starting_graph_nodes_positions + [(0, 0)]), 
                set(modified_graph_nodes_positions))
